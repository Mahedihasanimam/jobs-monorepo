...
import { formatJobDate, isExpired, toBanglaDigits } from '@/utils/date';
import { getJobShareMessage } from '@/utils/job';
...
export default function JobDetailsScreen() {
  ...
  if (!job || query.isError) return <SafeAreaView style={styles.safe}><Header onBack={router.back} /><EmptyState title="চাকরিটি পাওয়া যায়নি" message="তথ্যটি সরানো হয়ে থাকতে পারে বা সংযোগে সমস্যা হয়েছে।" actionLabel="আবার চেষ্টা করুন" onAction={() => void query.refetch()} /></SafeAreaView>;
  const expired = !job.is_active || isExpired(job.deadline);
  const applyLink = job.apply_url || job.source_url;
  const openPdf = () => { if (!getValidUrl(job.circular_url)) { Alert.alert('বিজ্ঞপ্তি পাওয়া যায়নি', 'এই চাকরির PDF বিজ্ঞপ্তির লিংক দেওয়া নেই।'); return; } router.push({ pathname: '/pdf', params: { url: job.circular_url!, title: job.title } }); };
  const eligibilitySummary = job.eligibility_summary || [job.education, job.experience, job.age_requirement].filter(Boolean).join('\n');
  const match = matchJob(job, profile);
  const fee = job.application_fee || job.description?.match(/(?:application fee|fee|আবেদন ফি|পরীক্ষার ফি)[^\n।]{0,60}/i)?.[0];

  return <SafeAreaView style={styles.safe} edges={['top', 'bottom']}><Header onBack={router.back} saved={saved} onSave={() => toggle(job.id)} onShare={() => void Share.share({ message: getJobShareMessage(job), title: job.title })} />
    <ScrollView contentContainerStyle={styles.content}>
      <View style={styles.hero}><View style={styles.orgRow}><OrganizationLogo name={job.organization} logoUrl={job.organization_logo_url} size={60} /><View style={styles.orgText}><Text style={styles.org}>{job.organization}</Text>{job.is_government_source ? <Text style={styles.verified}>যাচাইকৃত সরকারি উৎস</Text> : null}</View></View><Text style={styles.title}>{job.title}</Text><View style={styles.badges}><SourceBadge source={job.source} /><DeadlineBadge deadline={job.deadline} active={job.is_active} /></View></View>
      <View style={styles.matchCard}><View style={styles.matchHead}><View><Text style={styles.matchEyebrow}>আপনার সঙ্গে যোগ্যতার মিল</Text><Text style={styles.matchLabel}>{match.label}</Text></View><View style={styles.scoreBadge}><Text style={styles.scoreValue}>{toBanglaDigits(match.percentage)}%</Text></View></View>{job.circular_extraction_method ? <Text style={styles.extractionNote}>অফিসিয়াল বিজ্ঞপ্তির লেখা বিশ্লেষণ করে এই হিসাব করা হয়েছে।</Text> : <Text style={styles.extractionNote}>সংগৃহীত তথ্যের ভিত্তিতে হিসাব করা হয়েছে; বিজ্ঞপ্তি যাচাই করুন।</Text>}<View style={styles.matchDivider} /><Text style={styles.matchSection}>যে কারণে মিলেছে</Text>{match.reasons.map((reason) => <View key={reason} style={styles.reasonRow}><CheckCircle2 size={16} color={colors.primary} /><Text style={styles.matchGood}>{reason}</Text></View>)}<Text style={styles.matchSection}>যেসব শর্ত যাচাই করবেন</Text>{match.concerns.map((reason) => <View key={reason} style={styles.reasonRow}><AlertTriangle size={16} color="#9A6700" /><Text style={styles.matchConcern}>{reason}</Text></View>)}<View style={styles.nextAction}><Text style={styles.nextActionTitle}>পরবর্তী করণীয়</Text><Text style={styles.nextActionText}>{match.nextStep}</Text></View></View>
      <View style={styles.timelineCard}><View style={styles.timelineHead}><Text style={styles.timelineTitle}>আবেদনের সময়সীমা</Text><View style={[styles.status, expired && styles.statusExpired]}><CheckCircle2 size={13} color={expired ? colors.textSecondary : colors.primary} /><Text style={[styles.statusText, expired && styles.statusExpiredText]}>{expired ? 'সময় শেষ' : 'আবেদন চলছে'}</Text></View></View><View style={styles.timelineRow}><View style={styles.timelinePoint}><View style={styles.point} /><Text style={styles.timelineLabel}>প্রকাশ</Text><Text style={styles.timelineValue}>{formatJobDate(job.published_date) || 'উল্লেখ নেই'}</Text></View><View style={styles.timelineLine} /><View style={styles.timelinePoint}><View style={[styles.point, styles.pointEnd]} /><Text style={styles.timelineLabel}>শেষ সময়</Text><Text style={styles.timelineValue}>{formatJobDate(job.deadline) || 'উল্লেখ নেই'}</Text></View></View></View>
      <View style={styles.infoGrid}>
        {job.vacancies ? <InfoRow icon={Users} label="পদসংখ্যা" value={`${toBanglaDigits(job.vacancies)} পদ`} /> : null}
        {job.location ? <InfoRow icon={MapPin} label="কর্মস্থল" value={job.location} /> : null}
        {job.employment_type ? <InfoRow icon={BriefcaseBusiness} label="চাকরির ধরন" value={job.employment_type} /> : null}
      </View>
      {job.category ? <DetailSection title="চাকরির তথ্য" body={`ক্যাটাগরি: ${job.category}${job.employment_type ? `\nচাকরির ধরন: ${job.employment_type}` : ''}`} /> : null}
      {eligibilitySummary || job.eligible_applicants ? <View style={styles.requirementsCard}><View style={styles.requirementsHead}><View style={styles.requirementsIcon}><BookOpenCheck size={22} color={colors.primary} /></View><View style={styles.requirementsCopy}><Text style={styles.sectionTitle}>যোগ্যতার শর্ত</Text><Text style={styles.requirementsHint}>আপনার তথ্যের সঙ্গে প্রতিটি শর্ত মিলিয়ে দেখুন</Text></View></View><RequirementRow icon={GraduationCap} title="শিক্ষাগত যোগ্যতা" body={job.education} status={match.education} /><RequirementRow icon={BriefcaseBusiness} title="অভিজ্ঞতা" body={job.experience} status={match.experience} /><RequirementRow icon={BadgeCheck} title="বয়সসীমা" body={job.age_requirement} status={match.age} />{job.deadline ? <RequirementRow icon={FileText} title="আবেদনের শেষ তারিখ" body={formatJobDate(job.deadline)} status={expired ? 'mismatch' : 'match'} /> : null}{fee ? <RequirementRow icon={WalletCards} title="আবেদন ফি" body={fee} status="review" /> : null}{job.eligible_applicants ? <RequirementRow icon={UserCheck} title="আবেদন করতে পারবেন" body={job.eligible_applicants} status="review" /> : null}</View> : null}
      {!job.eligible_applicants && eligibilitySummary ? <DetailSection title="কারা আবেদন করতে পারবেন" body={`উপরের শিক্ষা, অভিজ্ঞতা ও বয়সের শর্ত পূরণকারী প্রার্থীরা আবেদন করতে পারবেন। চূড়ান্ত যোগ্যতার জন্য অফিসিয়াল সার্কুলার যাচাই করুন।`} /> : null}
      <DetailSection title="বেতন" body={job.salary} />
      <DetailSection title="বিস্তারিত" body={job.description} />
      {job.circular_url ? <View style={styles.pdfCard}><View style={styles.pdfHeading}><View style={styles.pdfCopy}><Text style={styles.sectionTitle}>অফিসিয়াল সার্কুলার</Text><Text style={styles.pdfHint}>নিচে বিজ্ঞপ্তির প্রিভিউ দেখুন</Text></View><Pressable accessibilityLabel="সার্কুলার পূর্ণ স্ক্রিনে দেখুন" style={styles.pdfExpand} onPress={openPdf}><Maximize2 size={17} color="#fff" /><Text style={styles.pdfExpandText}>পূর্ণ স্ক্রিনে দেখুন</Text></Pressable></View><View style={styles.pdfPreview}><CircularPdfViewer url={job.circular_url} /></View></View> : null}
...
function Header({ onBack, saved, onSave, onShare }: { onBack: () => void; saved?: boolean; onSave?: () => void; onShare?: () => void }) {
  return <View style={styles.header}><Pressable style={styles.headerButton} accessibilityLabel="পেছনে যান" onPress={onBack}><ChevronLeft size={25} color={colors.text} /></Pressable><Text style={styles.headerTitle}>চাকরির বিস্তারিত</Text><View style={styles.headerActions}>{onSave ? <Pressable style={styles.headerButton} accessibilityLabel="সংরক্ষণ করুন" onPress={onSave}><Bookmark size={21} color={saved ? colors.primary : colors.text} fill={saved ? colors.primary : 'transparent'} /></Pressable> : null}{onShare ? <Pressable style={styles.headerButton} accessibilityLabel="শেয়ার করুন" onPress={onShare}><Share2 size={21} color={colors.text} /></Pressable> : null}</View></View>;
}
function DetailSection({ title, body }: { title: string; body?: string | null }) {
  if (!body?.trim() || ['null', 'n/a', 'undefined'].includes(body.trim().toLowerCase())) return null;
  return <View style={styles.section}><Text style={styles.sectionTitle}>{title}</Text><Text selectable style={styles.body}>{body.trim()}</Text></View>;
}
function RequirementRow({ icon: Icon, title, body, status = 'review' }: { icon: typeof GraduationCap; title: string; body?: string | null; status?: MatchStatus }) {
  if (!body?.trim() || ['null', 'n/a', 'undefined'].includes(body.trim().toLowerCase())) return null;
  const tone = status === 'match' ? '#E5F7EF' : status === 'mismatch' ? '#FFF0F0' : '#FFF8E6'; const ink = status === 'match' ? colors.primary : status === 'mismatch' ? colors.red : '#A46800';
  return <View style={[styles.requirementRow, { backgroundColor: tone }]}><View style={styles.requirementIcon}><Icon size={18} color={ink} /></View><View style={styles.requirementCopy}><Text style={[styles.requirementTitle, { color: ink }]}>{title} · {status === 'match' ? 'মিলেছে' : status === 'mismatch' ? 'নাও মিলতে পারে' : 'যাচাই করুন'}</Text><Text selectable style={styles.requirementBody}>{body.trim()}</Text></View>{status !== 'match' ? <AlertTriangle size={16} color={ink} /> : null}</View>;
}
