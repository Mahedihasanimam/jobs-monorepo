...
const schema = z.object({
  ...
  education: z.string().optional(), subject: z.string().optional(), age: z.string().optional(), salary: z.string().optional(), applicationFee: z.string().optional(), experience: z.string().optional(), gender: z.string().optional(), freshersAllowed: z.boolean().optional(), minimumVacancies: z.string().optional(),
...
export default function FiltersScreen() {
  ...
  const { control, handleSubmit, reset, watch } = useForm<FilterForm>({ resolver: zodResolver(schema), defaultValues: { organization: filters.organization, category: filters.category, location: filters.location, employmentType: filters.employmentType, qualification: filters.qualification, education: filters.education, subject: filters.subject, age: filters.age, salary: filters.salary, applicationFee: filters.applicationFee, experience: filters.experience, gender: filters.gender, freshersAllowed: filters.freshersAllowed, minimumVacancies: filters.minimumVacancies ? String(filters.minimumVacancies) : '', deadlineRange: filters.deadlineRange ?? 'all', publishedRange: filters.publishedRange ?? 'all' } });
  const values = watch();
  const visibleActiveCount = [values.organization, values.category, values.location, values.employmentType, values.qualification, values.education, values.subject, values.age, values.salary, values.applicationFee, values.experience, values.gender, values.freshersAllowed, values.minimumVacancies, values.deadlineRange !== 'all' ? values.deadlineRange : null, values.publishedRange !== 'all' ? values.publishedRange : null].filter((value) => value !== undefined && value !== null && value !== '' && value !== false).length;
  const hiddenActiveCount = [filters.categoryKey, filters.womenEligible, filters.highSalary].filter(Boolean).length;
  const activeCount = visibleActiveCount + hiddenActiveCount;

  const apply = (values: FilterForm) => { const { minimumVacancies, ...rest } = values; setFilters({ ...filters, ...rest, minimumVacancies: minimumVacancies ? Number(minimumVacancies) : undefined, sort: filters.sort ?? 'latest' }); if (from === 'home') router.replace({ pathname: '/jobs', params: { filterApplied: String(Date.now()) } }); else router.back(); };
  const clear = () => {
    resetStore();
    reset({ organization: undefined, category: undefined, location: undefined, employmentType: undefined, qualification: undefined, education: undefined, subject: undefined, age: undefined, salary: undefined, applicationFee: undefined, experience: undefined, gender: undefined, freshersAllowed: false, minimumVacancies: '', deadlineRange: 'all', publishedRange: 'all' });
  };
  ...
  return <SafeAreaView style={styles.screen} edges={['top']}><View style={styles.compactTop}><Text style={styles.compactTitle}>ফিল্টার</Text>{activeCount ? <View style={styles.activeBadge}><Text style={styles.activeBadgeText}>{activeCount} সক্রিয়</Text></View> : <Text style={styles.compactHint}>প্রয়োজনীয় অপশন বেছে নিন</Text>}<Pressable accessibilityLabel="ফিল্টার বন্ধ করুন" style={styles.close} onPress={() => router.back()}><X size={21} color={colors.text} /></Pressable></View>
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <FilterSection title="যোগ্যতার শর্ত"><Controller control={control} name="education" render={({ field }) => <TextFilter label="শিক্ষা" placeholder="এসএসসি, এইচএসসি, স্নাতক…" value={field.value} onChange={field.onChange} />} /><Controller control={control} name="subject" render={({ field }) => <TextFilter label="বিষয়" placeholder="সিএসই, হিসাববিজ্ঞান…" value={field.value} onChange={field.onChange} />} /><Controller control={control} name="age" render={({ field }) => <TextFilter label="বয়স" placeholder="যেমন: 30" value={field.value} onChange={field.onChange} keyboard="number-pad" />} /><Controller control={control} name="experience" render={({ field }) => <TextFilter label="অভিজ্ঞতা" placeholder="যেমন: ২ বছর" value={field.value} onChange={field.onChange} />} /><Controller control={control} name="gender" render={({ field }) => <TextFilter label="লিঙ্গ" placeholder="পুরুষ বা নারী" value={field.value} onChange={field.onChange} />} /><Controller control={control} name="salary" render={({ field }) => <TextFilter label="বেতন" placeholder="যেমন: 30000" value={field.value} onChange={field.onChange} keyboard="number-pad" />} /><Controller control={control} name="applicationFee" render={({ field }) => <TextFilter label="আবেদন ফি" placeholder="যেমন: 500" value={field.value} onChange={field.onChange} keyboard="number-pad" />} /><Controller control={control} name="minimumVacancies" render={({ field }) => <TextFilter label="ন্যূনতম পদসংখ্যা" placeholder="যেমন: 10" value={field.value} onChange={field.onChange} keyboard="number-pad" />} /><Controller control={control} name="freshersAllowed" render={({ field }) => <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: field.value }} onPress={() => field.onChange(!field.value)} style={[styles.qualificationOption, field.value && styles.qualificationOptionActive]}><View style={[styles.qualificationCheck, field.value && styles.qualificationCheckActive]}>{field.value ? <Check size={15} color="#fff" /> : null}</View><Text style={styles.qualificationTitle}>নতুন প্রার্থীরা আবেদনযোগ্য</Text></Pressable>} /></FilterSection>
...
function TextFilter({ label, placeholder, value, onChange, keyboard = 'default' }: { label: string; placeholder: string; value?: string; onChange: (value: string) => void; keyboard?: 'default' | 'number-pad' }) { return <View style={styles.group}><Text style={styles.label}>{label}</Text><TextInput value={value} onChangeText={onChange} keyboardType={keyboard} placeholder={placeholder} placeholderTextColor={colors.textSecondary} style={styles.select} /></View>; }
